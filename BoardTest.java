...
public class BoardTest {
	...
	{ 
		...
		boolean result = board.isPieceThere(0,0); //Checking random spots cause I did not want to test them using the same for-loops I used to create them
		...
		assertEquals(y_initialPosition,intial_value_onBoard2);  //is 1, hence we see that the pawn's intital value is (0,1).   
		intial_value_onBoard1 =0; 
		intial_value_onBoard2 =1;
		 x_initialPosition =board.pieces[0].getCoordinates().get(0); 
		 y_initialPosition =board.pieces[0].getCoordinates().get(1);    // we see that the 17th piece i.e the rook is at position (0,0)
	...
	{   board.board_construct(8); 
	    ...
	    assertEquals(result, true);  //Testing check with queen
	} 
